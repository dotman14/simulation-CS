	...
	public static class UserConsumer
	{
		...
		private static void StudentConsumer(Queue<Student> studentQueue)
		{
			lock (SyncLock)
			{
				...
				Sql.LogUserCreation("UserConsume", student.GetType().Name, student.Name, DateTime.Parse(DateTime.Now.ToString("MM/dd/yy HH:mm:ss.ffffff")));
		...
		private static void FacultyConsumer(Queue<Faculty> facultyQueue)
		{
			lock (SyncLock)
			{
				...
				Sql.LogUserCreation("UserConsume", faculty.GetType().Name, faculty.Name, DateTime.Parse(DateTime.Now.ToString("MM/dd/yy HH:mm:ss.ffffff")));
